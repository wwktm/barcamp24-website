...
const match = configText.match(/LATEST_YEAR\s*=\s*(\d{4})/);
if (!match) {
...
const frozenDir = join(SRC, `ktm-${prevYear}`);
...
if (layout.includes("from '../config'")) {
  console.error(
    ...
    'Update the replace() in scripts/new-year.mjs to match the current import.'
  );
  process.exit(1);
}
